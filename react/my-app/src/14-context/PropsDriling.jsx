...
// Props Drilling(프롭스 드릴링)은 React와 같은 컴포넌트 기반 라이브러리에서 상위 컴포넌트의 데이터를 여러 중간 컴포넌트를 거쳐 최종적으로 필요한 하위 컴포넌트에 전달하는 과정을 말합니다.
// 예를 들어, 최상위 컴포넌트에서 데이터를 가지고 있다가, 그 데이터를 바로 사용하지 않는 중간 컴포넌트들을 통해 아주 깊은 곳에 있는 컴포넌트로 전달해야 할 때 프롭스 드릴링이 발생합니다.
